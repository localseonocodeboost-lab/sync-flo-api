...
function jsonError(request: NextRequest, message: string, status: number, details?: unknown) {
  return corsJson(request, { error: message, ...(details ? { details } : {}) }, status)
}
...
export async function POST(request: NextRequest) {
  ...
  if (!isNonEmptyString(apiKey)) {
    console.error("[SyncFlo] GOOGLE_PLACES_API_KEY is not configured on the server.")
  ...
  try {
    ...
    return jsonError(request, "Failed to reach the places provider.", 502)
  }
  ...
  if (!googleResponse.ok) {
    ...
    console.error("[SyncFlo] Google Places API error:", googleResponse.status, upstreamDetail)
    return jsonError(request, "Places provider returned an error.", 502)
  }
  ...
  try {
    ...
    return jsonError(request, "Received an invalid response from the places provider.", 502)
  }
  ...
  return corsJson(request, {
    query: { businessName, service, location, textQuery },
